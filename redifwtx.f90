...
! !INTERFACE:
      ...
      subroutine redifwtx(v,omeg,figu,wt)
...
!BOC
      ...
      case(4)                        ! for the case of none of them are equal
        ...
        do i=1,4
          ...
          enddo
        enddo
        if(dabs(omeg-v(2)).lt.haier) then
        ...
        else if(dabs(omeg-v(1)).lt.haier) then
     ...
     &       vdif(4,3))
          ...
          dd=(omeg-v(4))**3*vdif(2,1)**2*vdif(3,1)*vdif(3,2)**2
    
          aa=dlog(dabs(omeg-v(4)))*dd

          dd=(omeg-v(3))**3*vdif(2,1)**2*vdif(4,1)*vdif(4,2)**2

          aa=aa-dlog(dabs(omeg-v(3)))*dd

          dd=vdif(2,1)*vdif(3,2)*(omeg-v(4))-(omeg-v(1))*vdif(3,2)*   &
     &       vdif(4,2)
          dd=dd+(omeg-v(3))*vdif(2,1)*vdif(4,2)
          dd=dd*vdif(3,1)*vdif(4,1)*vdif(4,3)*(omeg-v(2))**2

          aa=aa+dlog(dabs(omeg-v(2)))*dd

          aa=aa-(omeg-v(2))**2*vdif(2,1)*vdif(3,1)*vdif(3,2)*         &
     &       vdif(4,1)*vdif(4,2)*vdif(4,3)

          aa=aa+dlog(dabs(omeg-v(1)))*(omeg-v(1))**3*vdif(3,2)**2*    &
     &       vdif(4,2)**2*vdif(4,3)
